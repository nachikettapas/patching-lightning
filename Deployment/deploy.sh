...
if [ "$NEW_INSTALL" = "1" ] && [ "$RBP" = "0" ]; then
    ...
    while IFS= read -r line
         do
           ...
           ssh -n $target "sudo apt-get update && sudo apt-get install -y autoconf automake build-essential git libtool libgmp-dev libsqlite3-dev python python3 net-tools zlib1g-dev libbase58-dev jq"
		   ssh -n $target "cd $HOME && wget "https://download.libsodium.org/libsodium/releases/LATEST.tar.gz" && tar -xvf ./LATEST.tar.gz && cd libsodium-stable && ./configure && make && make check && sudo make install"
